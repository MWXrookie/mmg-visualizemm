...
| **M2 划词精读** | 选中任意文字 → AI 判定角色（约束/目标/已知/假设/背景）+ 置信度 + **结构化角色卡** + 质疑重新论证 + 连续深挖 + **双向引用**（原文高亮） | ✅ |
| **M3 对话深挖** | 三步引导漏斗（①读题②精读③方向）+ **引导模式**（AI 反问引导不代做，可开关）+ 👍/👎/重新生成 | ✅ |
| **M4 知识卡片** | 决策树（SVG 逐层生长动画+深度滑块）/ 线性回归（多项式拟合+次数滑块）/ K-means（迭代动画+K 滑块）+ 对话概念自动触发入口 | ✅ |
| **M5 编程与沉淀** | 编程工作台（浏览器内 Python，Pyodide，matplotlib 出图内嵌）+ 会话自动保存/恢复 | ✅ |
| **BYOK** | 多服务商预设 + 自定义 OpenAI 兼容端点 + Key 浏览器加密存储 + 连接测试 | ✅ |
| 待办 | Docker 沙箱（可选）、代码编辑器增强、知识卡片库扩展、深色模式 | ⏳ |
