...
| **M6 知识库（RAG）** | **三源知识库**：方法库（24 种方法结构化）+ 知识卡片库（23 张概念）+ 获奖论文全文（15 篇），统一向量检索、按类型加权（方法库>卡片>论文）；**语义 embedding**（通义 text-embedding-v3，key 存 `server/.env.local`，失败降级本地哈希）；读题/建模/编程三台 AI 调用前自动检索注入上下文，回答可引用论文来源 | ✅ |
