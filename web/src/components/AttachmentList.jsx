...
export default function AttachmentList({ attachments, onRemove }) {
  ...
  return (
    <div className="att-list">
      ...
      {pending.map((a) => (
        <div key={a.id} className="att-chip">⏳ {a.name} 解析中…</div>
      ))}
      {failed.map((a) => (
        <div key={a.id} className="att-chip err">
          ⚠️ {a.name} 解析失败：{a.message}
          {onRemove && <button className="att-chip-del" onClick={() => onRemove(a.id)} title="删除附件">✕</button>}
        </div>
      ))}
    </div>
  )
  ...
  return (
    <div className="card text-card">
      <button className="text-head" onClick={() => setOpen(!open)}>
        <span>📄 {a.name} <span className="hint">文本附件</span></span>
        <span className="ctx-chev">{open ? '▲ 收起' : '▼ 展开'}</span>
      </button>
      {onRemove && (
